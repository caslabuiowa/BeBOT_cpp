...
public:
    ...
    virtual bool eval_g(Index n, const Number* x, bool new_x, Index m, Number* g) {
        ...
        DynamicMatrix<double> Dm_blaze(N_ + 1, N_ + 1); // Now correctly using the blaze namespace
        for (size_t i = 0; i < Dm.size(); ++i)
            for (size_t j = 0; j < Dm[i].size(); ++j)
                Dm_blaze(i, j) = Dm[i][j];

        //CustomVector<double, blaze::unaligned, blaze::unpadded> x_vector(x, N_ + 2);
        CustomVector<double, blaze::unaligned, blaze::unpadded> x_vector(const_cast<double*>(x), N_ + 1);

        auto g1 = trans(Dm_blaze) * x_vector;
        auto g2 = trans(Dm_blaze) * g1;
